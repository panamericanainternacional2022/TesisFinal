...
from core.models import (
    Usuario, Persona, Edificio, UsuarioEdificio,
    Notificacion, EquipoMonitoreo, EquipoSensor, DisposSensor,
    StatusEquipoMonitoreo, Status
)
...
REGEX_EMAIL = re.compile(r'^[a-zA-Z0-9]+(\.[a-zA-Z0-9]+)*@[a-zA-Z0-9]+(\.[a-zA-Z0-9]+)+$')
...
def _validaciones_formulario_usuario(data, exclude_persona_id=None):
    errores = {}
    campo = _validar_campo(data.get('primerNombre', ''), REGEX_SOLO_LETRAS, 'El primer nombre solo acepta letras.')
    if campo: errores['primerNombre'] = campo
    campo = _validar_longitud_max(data.get('primerNombre', ''), 20, 'El primer nombre')
    if campo: errores['primerNombre_long'] = campo
    campo = _validar_campo(data.get('segundoNombre', ''), REGEX_SOLO_LETRAS, 'El segundo nombre solo acepta letras.')
    if campo: errores['segundoNombre'] = campo
    campo = _validar_longitud_max(data.get('segundoNombre', ''), 20, 'El segundo nombre')
    if campo: errores['segundoNombre_long'] = campo
    campo = _validar_campo(data.get('primerApellido', ''), REGEX_SOLO_LETRAS, 'El primer apellido solo acepta letras.')
    if campo: errores['primerApellido'] = campo
    campo = _validar_longitud_max(data.get('primerApellido', ''), 20, 'El primer apellido')
    if campo: errores['primerApellido_long'] = campo
    campo = _validar_campo(data.get('segundoApellido', ''), REGEX_SOLO_LETRAS, 'El segundo apellido solo acepta letras.')
    if campo: errores['segundoApellido'] = campo
    campo = _validar_longitud_max(data.get('segundoApellido', ''), 20, 'El segundo apellido')
    if campo: errores['segundoApellido_long'] = campo
    campo = _validar_campo(data.get('cedula', ''), REGEX_SOLO_NUMEROS, 'La cédula solo acepta números.')
    if campo: errores['cedula'] = campo
    campo = _validar_longitud_max(data.get('cedula', ''), 8, 'La cédula')
    if campo: errores['cedula_long'] = campo
    campo = _validar_email(data.get('email', ''))
    if campo: errores['email'] = campo
    campo = _validar_telefono(data.get('telefono', ''))
    if campo: errores['telefono'] = campo
    campo = _validar_longitud_max(data.get('direccion', ''), 50, 'La dirección')
    if campo: errores['direccion_long'] = campo

    campo = _validar_unico_email(data.get('email', ''), exclude_persona_id)
    if campo: errores['email_unico'] = campo
    campo = _validar_unico_ci(data.get('cedula', ''), exclude_persona_id)
    if campo: errores['cedula_unico'] = campo
    campo = _validar_unico_telefono(data.get('telefono', ''), exclude_persona_id)
    if campo: errores['telefono_unico'] = campo
    return errores
...
def _validaciones_formulario_edificio(data, exclude_edificio_id=None):
    errores = {}
    campo = _validar_campo(data.get('nombreEdificio', ''), REGEX_SOLO_LETRAS, 'El nombre del edificio solo acepta letras.')
    if campo: errores['nombreEdificio'] = campo
    campo = _validar_longitud_max(data.get('nombreEdificio', ''), 20, 'El nombre del edificio')
    if campo: errores['nombreEdificio_long'] = campo
    campo = _validar_campo(data.get('direccion', ''), REGEX_DIRECCION, 'La dirección contiene caracteres no válidos.')
    if campo: errores['direccion'] = campo
    campo = _validar_longitud_max(data.get('direccion', ''), 50, 'La dirección')
    if campo: errores['direccion_long'] = campo
    campo = _validar_rif(data.get('rif', ''))
    if campo: errores['rif'] = campo
    campo = _validar_unico_rif(data.get('rif', ''), exclude_edificio_id)
    if campo: errores['rif_unico'] = campo
    return errores
...
def lista_usuario_view(request):
    ...
    usuarios = Usuario.objects.select_related('id_persona').prefetch_related('usuarioedificio_set__id_edificio').all()
    if query:
...
def editar_beneficiario_view(request, beneficiario_id):
    ...
    if request.method == 'POST':
        ...
        if not (primer_nombre and primer_apellido and email and cedula and id_edificio):
            messages.error(request, 'Complete los campos obligatorios (incluyendo edificio) para actualizar.')
        else:
...
def registro_edificio_view(request):
    ...
    if request.method == 'POST':
        ...
        if not (nombre and rif):
            bld_msgs.append({'text': 'Complete el nombre del edificio y el RIF.', 'type': 'error'})
            ...
            if errores_bld:
                ...
                bld_msgs.append({'text': 'Edificio registrado correctamente.', 'type': 'success'})
                request.session['_bld_msg'] = bld_msgs
...
def editar_edificio_view(request, edificio_id):
    ...
    if request.method == 'POST':
        ...
        rif = request.POST.get('rif', '').strip()
        if not (nombre and rif):
            bld_msgs.append({'text': 'Complete el nombre del edificio y el RIF para guardar los cambios.', 'type': 'error'})
            request.session['_bld_msg'] = bld_msgs
        else:
            errores_bld = _validaciones_formulario_edificio({
                'nombreEdificio': nombre,
            ...
            if errores_bld:
                ...
                edificio.rif = rif
                edificio.save()
                bld_msgs.append({'text': 'Edificio actualizado correctamente.', 'type': 'success'})
                request.session['_bld_msg'] = bld_msgs
                return redirect('lista_edificios')
    return render(request, 'pages/registro_edificio.html', {
        'editing': True,
...
def notificaciones_view(request):
    ...
    return render(request, 'pages/notificaciones.html', {
...
def monitoreo_view(request):
    ...
    if _is_admin_role(rol):
        edificios = Edificio.objects.all()
        return render(request, 'pages/monitoreo_admin.html', {
            'rol': rol,
            'edificios': edificios,
        })
    ...
    data = []
    for eq in equipos:
        ultimo_status = StatusEquipoMonitoreo.objects.filter(
            id_equipo_monitoreo=eq
        ).select_related('id_status').order_by('-id_status_equipo_monitoreo').first()

        sensores = EquipoSensor.objects.filter(
            id_equipo_monitoreo=eq
        ).select_related('id_dispos_sensor')

        data.append({
            'equipo': eq,
            'edificio': eq.id_edificio,
            'status': ultimo_status.id_status.nb_status if ultimo_status else 'Sin datos',
            'sensores': list(sensores),
        })

    return render(request, 'pages/monitoreo.html', {
        'equipos_data': data,
        'rol': rol,
        'query': query,
    })
...
def configuracion_view(request):
    ...
    if request.method == 'POST':
        ...
        if not password_ok:
            page_messages.append({'text': 'La contraseña actual no es correcta.', 'type': 'error'})
        ...
        if email:
            ...
            if err_email: errores_cfg['email'] = err_email
            else:
                err_email_unico = _validar_unico_email(email, exclude_persona_id=persona.id_persona)
                if err_email_unico: errores_cfg['email_unico'] = err_email_unico
        if username:
            err_user = _validar_campo(username, REGEX_USERNAME, 'El nombre de usuario solo acepta letras y números, sin espacios.')
            if err_user: errores_cfg['username'] = err_user
        if new_password:
            if len(new_password) < 6:
                errores_cfg['new_password'] = 'La contraseña debe tener al menos 6 caracteres.'
        ...
        usuario.save()
        request.session['usuario_username'] = usuario.username
        page_messages.append({'text': 'Configuración actualizada correctamente.', 'type': 'success'})
        request.session['_cfg_msg'] = page_messages
        return redirect('configuracion')
...
def seleccionar_usuario_view(request, accion):
    ...
    return render(request, 'pages/seleccionar_usuario.html', {
        'items': items,
        'accion': accion,
    })
...
def seleccionar_edificio_view(request, accion):
    ...
    return render(request, 'pages/seleccionar_edificio.html', {
        'items': items,
        'accion': accion,
    })
