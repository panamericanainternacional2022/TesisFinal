...
from front.sensor_config import VAR_NAMES, UNITS, DEVICE_NAMES_ES
...
def get_building_emails(edificio_id=None):
    try:
        ...
        from front.models import Edificio, EquipoMonitoreo, UsuarioEdificio
    except Exception:
        return []
    try:
        if not edificio_id:
...
def persist_notification_in_django(variable, value, risk_level, recommended_action, edificio_id=None):
    ...
    try:
        from django.utils import timezone
        from front.models import EquipoMonitoreo, Usuario, Notificacion
